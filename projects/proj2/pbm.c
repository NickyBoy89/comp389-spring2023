...
struct pbm_file *read_pbm_file(const char data[], int data_size) {
  ...
  if (num_indexes != MAX_WHITESPACE) {
    printf("Wrong number of whitespace characters found, expected %d, got %d\n",
           MAX_WHITESPACE, num_indexes);
    return NULL;
  }

  char *magic_temp = strndup(&data[0], indexes[0]);
  if (strcmp(magic_temp, PBM_MAGIC_NUMBER) != 0) {
...
void pbm_file_write_to_file(struct pbm_file *file, FILE *output) {
  ...
  for (int row_index = 0; row_index < file->height; row_index++) {
    fprintf(output, "%s", file->rows[row_index]);
  }
}
